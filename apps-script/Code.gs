/**
 * RAT-TRACK — Google Apps Script sync backend (v5)
 *
 * Sheet tabs:  Species | Shelves | Trays | Birth | Removal | Frozen
 *
...
var COL_LABEL_OVERRIDES = {
  trays:       { name: 'TRAY ID' },
  frozen_uses: { count: 'COUNT' }
};

var COL_LABELS = {
  id: '_ID', name: 'NAME', stages: '_STAGES_JSON',
  lifespan: 'LIFESPAN (DAYS)', gestation: 'GESTATION (DAYS)',
...
// Shelves      : TRAYS  (count of live trays)
// Trays        : SPECIES (3-letter code) + dynamic stage count cols
// Birth        : TRAY | SPECIES
// Removal      : TRAY
// Frozen       : SPECIES
var EXTRA = {
...
// Species      : id(1) name(2) stages_json(3) lifespan(4) gestation(5) updated(6) deleted(7) synced(8)
//                  | STAGES(9) SEX_RATIO(10) PINKY(11) FUZZY(12) HOPPER(13) ADULT(14)
...
function buildNameMaps() {
  ...
  return {
    ...
    speciesName: function(id) { return maps.species[String(id || '')] || singleSpeciesCode || ''; },
  ...
  incoming.forEach(function(r) {
    ...
    if (ctx) {
      if (entity === 'species') {
        ...
        rowValues.push(formatStages(stagesArr));
      ...
      if (entity === 'shelves')     rowValues.push('');
      ...
      if (entity === 'frozen_uses') rowValues.push(ctx.speciesName(r.speciesId));
    }
  ...
  SCHEMAS[entity].forEach(function(c) {
    ...
    if (JSON_FIELDS[c]) {
      ...
      v = (v === true || v === 'true' || v === 'TRUE');
                ...
                'updatedAt','syncedAt','lifespan','gestation','gravidFemales','lactatingFemales'].indexOf(c) >= 0) {
...
/**
 * Run ONCE from the Apps Script editor after pasting this v5 code.
 *
 * What it does:
 *  1. Renames tabs (species→Species, cohorts→Birth, etc.)
 *  2. Inserts GESTATION column in Species sheet if it is missing
 *  3. Inserts REASON column in Removal sheet if it is missing
 *  4. Creates the Frozen sheet if it is missing
 *  5. Re-applies HIDE on every sheet
 *  6. Recomputes shelf/tray stats
 */
function reformatSheets() {
  ...
  // 2. Insert GESTATION column in Species sheet (schema grew: old col 5 = _UPDATED → new col 5 = GESTATION)
  var speciesSh = ss.getSheetByName('Species');
  if (speciesSh && speciesSh.getLastColumn() >= 5) {
    var col5Hdr = String(speciesSh.getRange(1, 5).getValue());
    if (col5Hdr === '_UPDATED' || col5Hdr === 'UPDATED') {
      speciesSh.insertColumnBefore(5);
      speciesSh.getRange(1, 5).setValue('GESTATION (DAYS)')
        .setFontWeight('bold').setBackground('#1a1a2e').setFontColor('#ffffff').setFontSize(10);
    }
  }

  // 3. Remove old SHELF extra column from Trays if present
  var traysSh = ss.getSheetByName('Trays');
  if (traysSh && traysSh.getLastColumn() >= 10) {
    var h10 = String(traysSh.getRange(1, 10).getValue());
    if (h10 === 'SHELF') traysSh.deleteColumn(10);
  }

  // 4. Refresh extra resolved columns
  ...
  // 4a. Ensure Species extra columns exist (STAGES / SEX RATIO / PINKY / FUZZY / HOPPER / ADULT)
  var speciesSh2 = ss.getSheetByName(SHEET_NAMES.species);
  if (speciesSh2) {
    var spSchemaCols = SCHEMAS.species.length; // 8
    var spLastCol    = speciesSh2.getLastColumn();
    var spHdrs       = spLastCol > 0 ? speciesSh2.getRange(1, 1, 1, spLastCol).getValues()[0] : [];
    function ensureSpeciesCol(label) {
      for (var hi = 0; hi < spHdrs.length; hi++) {
        if (String(spHdrs[hi]) === label) return hi + 1;
      }
      var nc = speciesSh2.getLastColumn() + 1;
      speciesSh2.getRange(1, nc).setValue(label)
        .setFontWeight('bold').setBackground('#1a1a2e').setFontColor('#ffffff').setFontSize(10);
      spHdrs.push(label);
      return nc;
    }
    ...
    if (speciesSh2.getLastRow() > 1) {
      var spRows = speciesSh2.getRange(2, 1, speciesSh2.getLastRow() - 1, spSchemaCols).getValues();
      spRows.forEach(function(row, idx) {
        var rowNum  = idx + 2;
        var deleted = (row[6] === true || row[6] === 'true' || row[6] === 'TRUE'); // col 7 = deleted
        if (deleted) return;
        ...
        if (stageSummaryCol > 0) speciesSh2.getRange(rowNum, stageSummaryCol).setValue(stagesSummary);
        var sd = function(name) { var d = stageStartDay(stagesArr, name); return d !== null ? d + 'd+' : ''; };
        speciesSh2.getRange(rowNum, pinkyCol).setValue(sd('Pinky'));
        speciesSh2.getRange(rowNum, fuzzyCol).setValue(sd('Fuzzy'));
        speciesSh2.getRange(rowNum, hopperCol).setValue(sd('Hopper'));
        speciesSh2.getRange(rowNum, adultCol).setValue(sd('Adult'));
      });
    }
  }

  // 4b. Rename NAME → TRAY ID in Trays header
  var traysSh3 = ss.getSheetByName(SHEET_NAMES.trays);
  if (traysSh3) {
    var nameColIdx = SCHEMAS.trays.indexOf('name') + 1;
    if (nameColIdx > 0) {
      var hdrCell = traysSh3.getRange(1, nameColIdx);
      if (String(hdrCell.getValue()) === 'NAME') hdrCell.setValue('TRAY ID');
    }
    var fixedTrayCols = SCHEMAS.trays.length + (EXTRA.trays || []).length;
  ...
  // 4c. Insert REASON column in Removal sheet if missing
  ...
  // 4d. Create Frozen sheet if it doesn't exist
  sheetFor('frozen_uses');

  // 5. Re-apply HIDE on all sheets
  ENTITIES.forEach(function(entity) {
  ...
  try {
    SpreadsheetApp.getUi().alert(
      'Done!\n\n' +
      'Sheets: Species · Shelves · Trays · Birth · Removal · Frozen\n' +
      'GESTATION column added to Species.\n' +
      'All UUID and timestamp columns hidden.\n\n' +
      'Trigger a sync from the app to populate data.'
    );
  } catch(e) {}
}
